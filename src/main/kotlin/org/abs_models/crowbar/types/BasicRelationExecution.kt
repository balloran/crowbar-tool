...
/**
 * This class is the first try used for relational execution, many things are not working and it has been replaced by RelationExecution.kt.
 */

class BasicRelationExecution(private val filePaths: List<Path>){
    ...
    fun simpleExecute(){
        ...
        listNodeStrat.forEach { pair ->
            ...
            for(l in node.collectLeaves()){
                when(l){
                    is LogicNode -> {
                        val exec = AbstractEvaluation(listFraming[listNodeStrat.indexOf(pair)])
        ...
        when (val1){
            is FullAbstractTerm -> {
                ...
                if(val1.name != val2.name ||
                    ...
                    val1.accessibleValues.size != val2.accessibleValues.size){
                    return false
                }
                val list1 = val1.accessibleValues as MutableList
                val list2 = val2.accessibleValues as MutableList
            ...
            is ConcreteOnAbstractTerm -> {
                ...
                val map1 = mutableMapOf<ConcreteLocation, Term>()
                val map2 = mutableMapOf<ConcreteLocation, Term>()
    ...
    fun AEnormalize(data : Pair<AbstractTerm, List<Pair<ConcreteLocation, Term>>>): Pair<AbstractTerm, List<Pair<ConcreteLocation, Term>>>{
